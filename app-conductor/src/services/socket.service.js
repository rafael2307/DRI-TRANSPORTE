...
// Chequeo de bienestar/fatiga: solo llega si el conductor dio su
// consentimiento explícito (ver pantalla de ajustes). El backend nunca
// lo usa para penalizar, solo para preguntar cómo va el conductor.
onWellnessCheckIn(callback) {
    if (this.socket) {
        this.socket.on('wellnessCheckIn', callback);
    }
}

respondWellnessCheckIn(checkInId, response) {
    if (this.socket) {
        this.socket.emit('wellnessCheckInResponse', { checkInId, response });
    }
}

onWellnessCheckInAck(callback) {
    if (this.socket) {
        this.socket.on('wellnessCheckInAck', callback);
    }
}
